...
class MySelenium:
    ...
    def deleteBackupInChrome(self):
        ...
        for i in range(int(number_of_backups)):
            ...
            ai1wm_backup_dots = self.driver.find_element(
                By.CSS_SELECTOR,
                f"table.ai1wm-backups tr:nth-of-type({number_of_backups}) .ai1wm-backup-dots",
            )
            ai1wm_backup_dots.click()
            WebDriverWait(self.driver, 300000).until(
                EC.presence_of_element_located(
                    (
                        By.CSS_SELECTOR,
                        "table.ai1wm-backups tr:nth-of-type(2) .ai1wm-backup-delete",
                    )
                )
            )
            ai1wm_backup_delete = self.driver.find_element(
                By.CSS_SELECTOR,
                f"table.ai1wm-backups tr:nth-of-type({number_of_backups}) .ai1wm-backup-delete",
            )
